...
SELECT
    ...
    (SELECT COUNT(*) FROM comment c WHERE c.post_id = p.id AND c.del_flag = 0) AS comment_count,
    (SELECT COUNT(*) FROM post WHERE post_id = p.id) AS share_count,
    MAX(CASE WHEN r.user_id = ? THEN r.type ELSE NULL END) AS user_reaction_type,
    COALESCE(GROUP_CONCAT(DISTINCT ht.hashtag ORDER BY ht.hashtag), '') AS hashtags,
    (
        ...
        FROM Media sub_m
        WHERE sub_m.post_id = CASE WHEN p.is_share = 1 THEN p.post_id ELSE p.id END
    ) AS media_details,
    (
        SELECT JSON_ARRAYAGG(
            JSON_OBJECT(
...
WHERE
    ...
    AND p.delflag = 0
    GROUP BY
    p.id, p.user_id, p.location_id, l.address, u.fullname, u.avatar_url,
    p.content, p.status, p.post_id, p.create_time,
    p.is_share, p.share_by_id, us.fullname, us.avatar_url,
    p.share_content, p.share_time, p.status_share
ORDER BY
    p.create_time DESC -- Sắp xếp theo thời gian mới nhất
LIMIT ? OFFSET ?;

`;

module.exports = { queryPostById, queryPostOfUser, queryPostGlobal };
