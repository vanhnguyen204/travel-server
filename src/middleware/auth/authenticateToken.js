...
const secretKey =
  "/q5Il7oI//Hiv4va97MQAtYOaktNo188-23WY12YVRCRGBEwYECRg0T6YcrEzYWb";
...
const authenticateToken = async (req, res, next) => {
  ...
  if (!token) {
    return res.status(401).json({
      status: 401,
      message: "Không có quyền truy cập vì thiếu token.",
  ...
  try {
    ...
    const [checkUserIsLock] = await pool
      .promise()
      .query("Select * from user where email = ? ", decoded.iss);
    ...
    res.status(401).json({
      status: 401,
      message: "Xác thực tài khoản thất bại, vui lòng thử lại sau.",
      cause:
